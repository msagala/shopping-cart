...
It is a use case where you will add an item to an existing cart, you need to input a cart_id to use that cart to add the new item or update the quantity of an existing item. If no cart found, it will return an error "Cart not found!".
